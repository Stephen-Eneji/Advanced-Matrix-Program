...
def conditionsfortype():
    if matrixTypeselector.isdigit():

        int_matrixTypeselector = int(matrixTypeselector)
        if int_matrixTypeselector == 1:
            return logicfor2x2()
        if int_matrixTypeselector ==2:
            return logicfor2x3()
        if int_matrixTypeselector == 3:
            return logicfor3x3()
        if int_matrixTypeselector == 4:
            return logicfor3x2()
    else:
        print("Input 1, 2, 3 or 4")

def Operationselector():
    if operationSelector ==str():
        if operationSelector == ['S', 's']:
            return SUM()
        if operationSelector == ['D', 'd']:
            return DIFFERENCE()

    else:
        print("Do you want to Add, Subtract or find the product of a matrix?")
conditionsfortype()
